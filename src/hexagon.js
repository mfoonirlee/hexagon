...
let HexagonComponent = React.createClass({
    ...
    setChosedStatus: function (id, localState) {
        let chosedItemsList = localState.items.filter(function(n){return n.isChosed && !n.isHide}),
            chosedArr = localState.chosedArr,
            item = localState.items.find(function(n){return n.id == id});
        ...
        if(item.isChosed){
            ...
            //选择已选择的item表示取消选择
            let index;
            chosedArr.forEach(function(n, i){if(n.id == item.id){index = i;}});
    ...
    handleClick: function (index, target) {
        var target = target.currentTarget,
            ...
            item = this.state.items.find(function(n){return n.id == id}),
            ...
            <div className='hexagon-container'>
                {
                    this.state.items.map(function(item, i){
                        ...
                        return (
                            <div className={className} onClick={this.handleClick.bind(this, i)} key={i} data-text={item.text} data-type={item.type} data-id={item.id}><div className='circle'><p>{item.text}</p></div></div>
                        );
                    }, this)
                }
            </div>
